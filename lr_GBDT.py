...
testDf = pd.read_csv(path+train_file,nrows=1000,skiprows=(1,10000))
...
def do_model_metric(y_true,y_pred,y_pred_prob):
    ...
    print("Lable 1 percent: {0}".format(train[label].mean))
...
#3.2 GBDT
lgb_feates = con_feats+bin_feats+cat_feats
categorical_feature_lists = cat_feats + bin_feats
...
lgbtrain = lgb.Dataset(train[lgb_feates].values,label=train[label].values,feature_name=lgb_feates,categorical_feature=categorical_feature_lists)
lgbvalid = lgb.Dataset(test[lgb_feates].values,label = test[label].value,feature_name=lgb_feates,categorical_feature=categorical_feature_lists)
...
# 得到叶子节点的编号Features transformation
gdbt_feats_vals = lgb_model.predict(combineDf[lgb_feates],pred_leaf=True)
gdbt_columns = ["gdbt_leaf_indices_"+str(i) for i in range(0,gdbt_feats_vals.shape[1])]

# combineDf = pd.concat([combineDf,pd.DataFrame(data=gdbt_feats_vals,index=range(0,gdbt_feats_vals.shape[0],columns = gdbt_columns)],axis=1)
combineDf = pd.concat([combineDf, pd.DataFrame(data=gdbt_feats_vals, index=range(0, gdbt_feats_vals.shape[0]),columns=gdbt_columns)], axis=1)

#onehotencode(gebt_feats)
origin_columns = combineDf.columns;
for col in gdbt_columns:
    combineDf = pd.concat([combineDf,pd.get_dummies(combineDf[col],prefix=col)],axis=1)

gdbt_onehot_feats = [col for col in combineDf.columns if col not in origin_columns]
...
lr_gbdt_feats = lr_feats + gdbt_onehot_feats
...
print("Test")
do_model_metric(y_true=test[label],y_pred=lr_gbdt_model.predict(train[lr_gbdt_feats],lr_gbdt_model.predict_proba(train[lr_gbdt_feats])))
